...
const remainingSpace = TOTAL_SPACE - getTotalFileSizesForDir(rootDir);
const minDeletableSize = 30000000 - remainingSpace;

let smallestDeletableDirSize = TOTAL_SPACE;
function findSmallestDeletableDirSize(dir: Dir) {
    dir.children.forEach((child) => {
        if (child.type === 'dir') {
            findSmallestDeletableDirSize(child);
            const dirSize = getTotalFileSizesForDir(child);
            if (dirSize > minDeletableSize && dirSize < smallestDeletableDirSize) {
                smallestDeletableDirSize = dirSize;
            }
        }
    });
}

findSmallestDeletableDirSize(rootDir);

console.log(smallestDeletableDirSize);
